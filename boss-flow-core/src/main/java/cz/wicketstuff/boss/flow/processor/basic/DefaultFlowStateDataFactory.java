...
public class DefaultFlowStateDataFactory<T extends Serializable> implements IFlowStateDataFactory<T>, Serializable {

	private static final long serialVersionUID = 1L;
	
	private Map<IFlowState, Class<Serializable>> stateDataMap;	
	private boolean exceptionIfStateNotExist;
	
	public DefaultFlowStateDataFactory() {
		this(null);
	}

	public DefaultFlowStateDataFactory(
			Map<IFlowState, Class<Serializable>> stateDataMap) {
	...
	public Serializable createStateData(IFlowCarter<T> flowCarter, IFlowState flowState) throws NoSuchStateException, StateDataException {
		Class<Serializable> c = getStateDataMap().get(flowState);
		if(c == null) {
	...
	public Map<IFlowState, Class<Serializable>> getStateDataMap() {
		if(stateDataMap == null) {
			stateDataMap = new HashMap<IFlowState, Class<Serializable>>();
		}
		return stateDataMap;
	}

	public void setStateDataMap(Map<IFlowState, Class<Serializable>> stateDataMap) {
		this.stateDataMap = stateDataMap;
	}
